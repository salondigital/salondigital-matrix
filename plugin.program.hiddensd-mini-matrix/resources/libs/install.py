...
def wipe():
    ...
    progress_dialog.create(CONFIG.ADDONTITLE, "[COLOR {0}]Calculando archivos y carpetas".format(CONFIG.COLOR2) + '\n' + '\n' + 'Por favor espere![/COLOR]')
    total_files = sum([len(files) for r, d, files in os.walk(xbmcPath)])
    del_file = 0
    progress_dialog.update(0, "[COLOR {0}]Obteniendo listado de exclusiones.[/COLOR]".format(CONFIG.COLOR2))
    ...
    progress_dialog.update(0, "[COLOR {0}]Limpiando archivos y carpetas:".format(CONFIG.COLOR2))
    ...
    for root, dirs, files in os.walk(xbmcPath, topdown=True):
        ...
        if progress_dialog.iscanceled():
            ...
            logging.log_notify(CONFIG.ADDONTITLE,
                               "[COLOR {0}]Instalacion a Kodi por defecto cancelada[/COLOR]".format(CONFIG.COLOR2))
            return False
    for root, dirs, files in os.walk(xbmcPath, topdown=True):
        dirs[:] = [d for d in dirs if d not in exclude_dirs]
        for name in dirs:
            progress_dialog.update(100, '\n' + 'Limpiando carpetas vacias: [COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR1, name))
...
def fresh_start(install=None, over=False):
    ...
    if over:
        ...
        yes_pressed = dialog.yesno(CONFIG.ADDONTITLE,
                                       "[COLOR {0}]Quieres restaurar tu".format(CONFIG.COLOR2)
                                       +'\n'+"Configuracion de Kodi a valores por defecto"
                                       +'\n'+"antes de instalar esta version?[/COLOR]",
                                       nolabel='[B][COLOR red]No, Cancelar[/COLOR][/B]',
                                       yeslabel='[B][COLOR springgreen]Continuar[/COLOR][/B]')
    elif install:
        yes_pressed = dialog.yesno(CONFIG.ADDONTITLE, "[COLOR {0}]Quieres restaurar tu".format(CONFIG.COLOR2)
                                       +'\n'+"Configuracion de Kodi a valores por defecto"
                                       +'\n'+"antes de instalar esta version? [COLOR {0}]{1}[/COLOR]?".format(CONFIG.COLOR1, install),
                                       nolabel='[B][COLOR red]No, Cancelar[/COLOR][/B]',
                                       yeslabel='[B][COLOR springgreen]Continuar[/COLOR][/B]')
    else:
        yes_pressed = dialog.yesno(CONFIG.ADDONTITLE, "[COLOR {0}]Quieres restaurar tu".format(CONFIG.COLOR2) +' \n' + "Configuracion de Kodi a valores por defecto=[/COLOR]", nolabel='[B][COLOR red]No, Cancelar[/COLOR][/B]', yeslabel='[B][COLOR springgreen]Continuar[/COLOR][/B]')
    if yes_pressed:
        wipe()
        ...
        if over:
            ...
            dialog.ok(CONFIG.ADDONTITLE, "[COLOR {0}]Para guardar los cambios necesitas cerrar Kodi, pulsa OK para forzar el cierre de Kodi[/COLOR]".format(CONFIG.COLOR2))
            from resources.libs import update
...
def install_apk(name, url):
    ...
    if tools.platform() == 'android':
        ...
        if os.path.exists(lib):
            redownload = dialog.yesno(CONFIG.ADDONTITLE, '[COLOR {}]{}[/COLOR] ya existe. Quieres volver a descargartelo?'.format(CONFIG.COLOR1, apk),
                               yeslabel="[B]Descargar de nuevo[/B]",
                               nolabel="[B]Instalar[/B]")
            yes = False
        else:
            yes = dialog.yesno(CONFIG.ADDONTITLE,
                                   "[COLOR {0}]Quieres descargar e instalar: ".format(CONFIG.COLOR2)
            ...
            if not yes:
                logging.log_notify(CONFIG.ADDONTITLE,
                               '[COLOR {0}]ERROR: Instalacion cancelada[/COLOR]'.format(CONFIG.COLOR2))
                return
        ...
        if yes or redownload:
            ...
            if not response:
                logging.log_notify(CONFIG.ADDONTITLE,
                                   '[COLOR {0}]Instalador de APK: URL de APK no valida![/COLOR]'.format(CONFIG.COLOR2))
                return
        ...
        dialog.ok(CONFIG.ADDONTITLE, '[COLOR {}]{}[/COLOR] descargado a [COLOR {}]{}[/COLOR]. Si la instalacion no se inicia por si sola, navegue hasta la ruta e instale el APK manualmente'.format(CONFIG.COLOR1, apk, CONFIG.COLOR1, path))
        ...
        logging.log_notify(CONFIG.ADDONTITLE,
                           '[COLOR {0}]ERROR: No es un dispositivo Android[/COLOR]'.format(CONFIG.COLOR2))
