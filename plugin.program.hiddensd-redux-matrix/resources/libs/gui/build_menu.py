...
class BuildMenu:
    ...
    def get_listing(self):
        ...
        if response:
            ...
            directory.add_file('Version de Kodi: {0}'.format(CONFIG.KODIV), icon=CONFIG.ICONBUILDS,
                               themeit=CONFIG.THEME3)
            directory.add_dir('Guardar datos', {'mode': 'savedata'}, icon=CONFIG.ICONSAVE, themeit=CONFIG.THEME3)
            directory.add_separator()
            directory.add_file('Error leyendo el archivo txt', icon=CONFIG.ICONBUILDS, themeit=CONFIG.THEME3)
        ...
        directory.add_file('Version de Kodi: {0}'.format(CONFIG.KODIV), icon=CONFIG.ICONBUILDS, themeit=CONFIG.THEME3)
        directory.add_dir('Guardar datos', {'mode': 'savedata'}, icon=CONFIG.ICONSAVE, themeit=CONFIG.THEME3)
        directory.add_separator()
        ...
        if len(match) >= 1:
            if CONFIG.SEPARATE == 'true':
                ...
                if count19 > 0:
                    ...
                    directory.add_file('[B]{0} Version para Matrix ({1})[/B]'.format(state, count19), {'mode': 'togglesetting',
            ...
            if adultcount > 0:
                directory.add_file('Solo hay addons para adultos', icon=CONFIG.ICONBUILDS,
                                   themeit=CONFIG.THEME3)
                directory.add_file('Activa addons de adultos en Addon Settings > Misc', icon=CONFIG.ICONBUILDS,
                                   themeit=CONFIG.THEME3)
            else:
                directory.add_file('No hay versiones disponibles desde {0}'.format(CONFIG.ADDONTITLE),
                                   icon=CONFIG.ICONBUILDS, themeit=CONFIG.THEME3)
        else:
            directory.add_file('Archivo de versiones no formateado correctamente.', icon=CONFIG.ICONBUILDS,
                               themeit=CONFIG.THEME3)
    ...
    def view_build(self, name):
        ...
        if response:
            ...
            directory.add_file('Error leyendo el archivo txt', themeit=CONFIG.THEME3)
            directory.add_file('{0}'.format(CONFIG.BUILDFILE), themeit=CONFIG.THEME3)
            return

        if not check.check_build(name, 'version'):
            directory.add_file('Error leyendo el archivo txt.', themeit=CONFIG.THEME3)
            directory.add_file('{0} no esta disponible para instalar.'.format(name), themeit=CONFIG.THEME3)
            return
        ...
        for version, url, gui, kodi, themefile, icon, fanart, preview, adult, info, description in match:
            ...
            directory.add_separator()
            directory.add_dir('Guardar datos', {'mode': 'savedata'}, icon=CONFIG.ICONSAVE, themeit=CONFIG.THEME3)
            directory.add_file('Informacion de la version', {'mode': 'buildinfo', 'name': name}, description=description, fanart=fanart,
                               icon=icon, themeit=CONFIG.THEME3)
                               
            if previewcheck:
                directory.add_file('Ver preview en video', {'mode': 'buildpreview', 'name': name}, description=description, fanart=fanart,
                                   icon=icon, themeit=CONFIG.THEME3)
            
            if versioncheck:
                directory.add_file(
                    '[I]Version creada para Kodi v{0} (instalada: v{1})[/I]'.format(str(kodi), str(CONFIG.KODIV)),
    ...
    def build_info(self, name):
        ...
        if response:
            if check.check_build(name, 'url'):
                ...
                msg = "[COLOR {0}]Nombre de version:[/COLOR] [COLOR {1}]{2}[/COLOR][CR]".format(CONFIG.COLOR2, CONFIG.COLOR1, name)
                msg += "[COLOR {0}]Numero de version:[/COLOR] [COLOR {1}]{2}[/COLOR][CR]".format(CONFIG.COLOR2, CONFIG.COLOR1, version)
                if themes:
                    msg += "[COLOR {0}]Tema:[/COLOR] [COLOR {1}]{2}[/COLOR][CR]".format(CONFIG.COLOR2, CONFIG.COLOR1, ', '.join(themes))
                msg += "[COLOR {0}]Version de Kodi:[/COLOR] [COLOR {1}]{2}[/COLOR][CR]".format(CONFIG.COLOR2, CONFIG.COLOR1, kodi)
                ...
                if extend:
                    ...
                    msg += "[COLOR {0}]Peso de los archivos extraidos:[/COLOR] [COLOR {1}]{2}[/COLOR][CR][CR]".format(CONFIG.COLOR2, CONFIG.COLOR1, tools.convert_size(int(float(extracted))))
                    msg += "[COLOR {0}]Peso del archivo zip:[/COLOR] [COLOR {1}]{2}[/COLOR][CR][CR]".format(CONFIG.COLOR2, CONFIG.COLOR1, tools.convert_size(int(float(zipsize))))
                    msg += "[COLOR {0}]Nombre de la skin:[/COLOR] [COLOR {1}]{2}[/COLOR][CR][CR]".format(CONFIG.COLOR2, CONFIG.COLOR1, skin)
                ...
                window.show_text_box("Ver informacion de la version: {0}".format(name), msg)
            else:
                logging.log("Nombre de la version no valido!")
        else:
            logging.log("Archivo de la version no funcional: {0}".format(CONFIG.BUILDFILE))
    ...
    def build_video(self, name):
        ...
        if response:
            ...
            if tools.open_url(videofile, check=True):
                ...
                logging.log("[{0}]No hay url de preview".format(name))
        else:
            logging.log("Archivo de la version no funcional: {0}".format(CONFIG.BUILDFILE))
    ...
    def create_install_menu(self, name):
        ...
        menu_items.append((CONFIG.THEME3.format('Instalacion de fabrica'), 'RunPlugin(plugin://{0}/?mode=install&name={1}&url=fresh)'.format(CONFIG.ADDON_ID, buildname)))
        ...
        menu_items.append((CONFIG.THEME3.format('Aplica guiFix'), 'RunPlugin(plugin://{0}/?mode=install&name={1}&url=gui)'.format(CONFIG.ADDON_ID, buildname)))
        menu_items.append((CONFIG.THEME3.format('Informacion de la version'), 'RunPlugin(plugin://{0}/?mode=buildinfo&name={1})'.format(CONFIG.ADDON_ID, buildname)))
        menu_items.append((CONFIG.THEME2.format('{0} Opciones'.format(CONFIG.ADDONTITLE)), 'RunPlugin(plugin://{0}/?mode=settings)'.format(CONFIG.ADDON_ID)))
